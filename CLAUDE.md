...
- **mcp-server-nav-language** — Pure regex MCP server (sub-10ms) covering the same pattern space as this plugin but for any language. Used by Gary MCP hub Phase 3 for runtime enforcement. This ESLint plugin handles JS/TS at dev time; the MCP server handles runtime checks across the agent swarm.
